...
# Python program for Kruskal's algorithm to find Minimum ST of a given connected, undirected and weighted graph
...
class Graph:
    ...
    def maximum_spanning_tree(self):
        ...
        # Step 1:  Sort all the edges in non-INCREASING order of their weight.
        # If we are not allowed to change the given graph, we can create a copy of graph
        self.graph = sorted(self.graph, key=lambda item: item[2])[::-1]
        ...
        while e < self.V - 1:
            ...
            # Step 2: Pick the LARGEST edge and increment the index for next iteration
            u, v, w = self.graph[i]
            i = i + 1
            x = self.find(parent, u)
            y = self.find(parent, v)
